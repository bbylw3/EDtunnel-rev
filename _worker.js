...
export default {
    ...
    async fetch(request, env, ctx) {
        try {
            ...
            // Check if it's a direct access to the root path
            if (url.pathname === '/') {
            ...
            if (!upgradeHeader || upgradeHeader !== 'websocket') {
                switch (url.pathname) {
                    case `/cf`: {
                        // Return the Cloudflare object information
                    ...
                    case `/${userID_Path}`: {
                        // Return the generated vless configuration
                    ...
                    case `/bestip/${userID_Path}`: {
                        // Request the best IP configuration and return
                    ...
                    default:
                        // For other paths, proxy the request to a random site
                        ...
                        // Send the proxy request and handle redirects
...
function createVlessSub(userID_path, hostname) {
	...
	const result = userIDArray.flatMap((userID) => {
		const configHttp = Array.from(httpPortSet).flatMap((port) => {
		...
		const configHttps = Array.from(httpsPortSet).flatMap((port) => {
